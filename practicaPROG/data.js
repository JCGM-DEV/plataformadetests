...
const QUIZZES = {
  poo_quiz: { title: 'Examen Completo de Programación', questions: [
    ...
    { q: '¿Qué palabra clave se usa para llamar al constructor de la clase padre?', hint: 'Se usa en la primera línea del constructor hijo.',
      opts: ['this()', 'parent()', 'super()', 'base()'], ans: 2,
      exp: '<strong>super()</strong> llama al constructor de la superclase. Debe ser siempre la primera instrucción del constructor de la subclase.' },
    { q: '¿Qué es el polimorfismo en Java?', hint: 'Relacionado con tratar diferentes objetos como si fueran del mismo tipo base.',
      opts: ['La capacidad de un objeto de tomar muchas formas', 'Ocultar datos', 'Crear múltiples clases en un fichero', 'Heredar de muchas clases'], ans: 0,
      exp: 'El <strong>polimorfismo</strong> permite que una referencia de una clase padre apunte a un objeto de una subclase, permitiendo ejecutar métodos sobreescritos.' },
    { q: '¿Se puede instanciar una clase abstracta?', hint: 'Es una clase incompleta.',
      opts: ['Sí, como cualquier otra', 'Sí, pero solo una vez', 'No, nunca', 'Solo si no tiene métodos abstractos'], ans: 2,
      exp: 'Una <strong>clase abstracta</strong> NO se puede instanciar (no puedes hacer new de ella). Sirve como base para otras clases.' },
    { q: '¿Qué método de la clase Object compara si dos objetos tienen la misma referencia de memoria?', hint: 'Se usa con == por defecto.',
      opts: ['compare()', 'equals()', 'hash()', 'toString()'], ans: 1,
      exp: 'El método <strong>equals()</strong> de Object compara referencias. Muchas clases (como String) lo sobreescriben para comparar contenido.' },
    { q: '¿Cuál es la principal ventaja de un ArrayList sobre un array convencional?', hint: 'Sobre el tamaño.',
      opts: ['Es más rápido', 'Gasta menos memoria', 'Su tamaño es dinámico', 'Solo admite objetos'], ans: 2,
      exp: 'El <strong>ArrayList</strong> puede crecer o encogerse dinámicamente según sea necesario, a diferencia de los arrays de tamaño fijo.' },
    { q: '¿Qué colección garantiza que no haya elementos duplicados?', hint: 'Basado en conjuntos matemáticos.',
      opts: ['ArrayList', 'HashMap', 'HashSet', 'LinkedList'], ans: 2,
      exp: 'Un <strong>Set</strong> (como HashSet) no permite duplicados. Si intentas añadir uno, simplemente no lo guarda.' },
    { q: 'En un HashMap, ¿qué ocurre si insertas una clave que ya existe?', hint: 'put(clave, valor).',
      opts: ['Lanza una excepción', 'Se ignora la inserción', 'Se sobreescribe el valor anterior para esa clave', 'Se crean dos entradas'], ans: 2,
      exp: 'En <strong>HashMap</strong>, las claves son únicas. Si usas put() con una clave existente, el valor antiguo se reemplaza por el nuevo.' },
    { q: '¿Qué método devuelve el número de elementos en un ArrayList?', hint: 'No es length.',
      opts: ['length()', 'count()', 'size()', 'total()'], ans: 2,
      exp: 'En colecciones se usa <strong>size()</strong>. .length se usa en arrays y .length() en Strings.' },
    { q: '¿Qué interfaz permite ordenar una lista de objetos de forma personalizada?', hint: 'Recibe dos parámetros (o1, o2).',
      opts: ['Sortable', 'Comparable', 'Comparator', 'Orderable'], ans: 2,
      exp: '<strong>Comparator</strong> define el método compare(o1, o2) para ordenaciones externas o personalizadas. Comparable define compareTo(o) dentro de la propia clase.' },
    { q: '¿Cuál es la complejidad temporal de buscar un elemento por clave en un HashMap?', hint: 'Es extremadamente rápido.',
      opts: ['O(1) - Constante', 'O(n) - Lineal', 'O(log n)', 'O(n^2)'], ans: 0,
      exp: 'El acceso por clave en un <strong>HashMap</strong> es O(1) de media gracias a la tabla hash.' },
    { q: '¿Qué hace el método Collections.sort()?', hint: 'Funciona sobre List.',
      opts: ['Ordena un array', 'Ordena una lista basándose en el orden natural o un Comparator', 'Borra duplicados', 'Invierte la lista'], ans: 1,
      exp: '<strong>Collections.sort()</strong> ordena listas. Los elementos deben implementar Comparable o pasarle un Comparator.' },
    { q: '¿Qué ocurre si intentas acceder al índice 5 de un ArrayList de tamaño 3?', hint: 'Un error muy común.',
      opts: ['Devuelve null', 'Devuelve 0', 'Lanza IndexOutOfBoundsException', 'El ArrayList crece automáticamente'], ans: 2,
      exp: 'Acceder a un índice inexistente lanza <strong>IndexOutOfBoundsException</strong>. El crecimiento automático solo ocurre al añadir, no al leer.' },
    { q: '¿Qué palabra clave se usa para capturar una excepción?', hint: 'Parte del bloque try-catch.',
      opts: ['catch', 'throw', 'throws', 'finally'], ans: 0,
      exp: 'El bloque <strong>catch</strong> contiene el código que se ejecuta si ocurre una excepción en el try.' },
    { q: '¿Cuándo se ejecuta el bloque "finally"?', hint: 'Garantía de ejecución.',
      opts: ['Solo si hay error', 'Solo si no hay error', 'Siempre, ocurra o no una excepción', 'Solo si el sistema tiene memoria'], ans: 2,
      exp: 'El bloque <strong>finally</strong> se ejecuta siempre, ideal para cerrar recursos como ficheros o conexiones.' },
    { q: '¿Qué diferencia hay entre "throw" y "throws"?', hint: 'Lanzar vs declarar.',
      opts: ['Son iguales', 'throw lanza la excepción; throws la declara en la firma del método', 'throw es para errores graves; throws para leves', 'throws lanza; throw declara'], ans: 1,
      exp: '<strong>throw</strong> se usa para lanzar una instancia de excepción. <strong>throws</strong> se pone en la cabecera del método para avisar que puede lanzar esa excepción.' },
    { q: '¿Qué es una excepción de tipo "checked" (comprobada)?', hint: 'El compilador te obliga a gestionarla.',
      opts: ['Errores de lógica como dividir por cero', 'Excepciones que heredan de Exception pero no de RuntimeException', 'Errores fatales de la JVM', 'Excepciones que nunca ocurren'], ans: 1,
      exp: 'Las <strong>Checked Exceptions</strong> (como IOException) deben ser capturadas o declaradas obligatoriamente en el código.' },
    { q: '¿Cuál es la superclase de todas las excepciones?', hint: 'Todo lo que se puede lanzar.',
      opts: ['Exception', 'Error', 'Throwable', 'RuntimeException'], ans: 2,
      exp: '<strong>Throwable</strong> es la raíz. De ella cuelgan Error (irrecuperables) y Exception (recuperables).' },
    { q: '¿Qué ocurre si hay un "return" dentro de un bloque try y también hay un bloque finally?', hint: 'El profesor va con mala leche.',
      opts: ['No se ejecuta el finally', 'Se ejecuta el finally antes de devolver el valor', 'Error de compilación', 'El return se ignora'], ans: 1,
      exp: 'El bloque <strong>finally</strong> se ejecuta SIEMPRE, incluso si hay un return justo antes. Primero se evalúa la expresión del return, luego se ejecuta el finally, y luego sale.' },
    { q: '¿Qué excepción se lanza al intentar llamar a un método de un objeto que es null?', hint: 'La más famosa de Java.',
      opts: ['ClassCastException', 'NullPointerException', 'IllegalArgumentException', 'NoSuchMethodException'], ans: 1,
      exp: '<strong>NullPointerException (NPE)</strong>: ocurre al intentar acceder a miembros de una referencia que no apunta a ningún objeto.' },
    { q: '¿Cómo se crea una excepción personalizada?', hint: 'Aprovechando la jerarquía.',
      opts: ['Creando una interfaz', 'Heredando de Exception o RuntimeException', 'Usando la palabra new Exception', 'No se puede'], ans: 1,
      exp: 'Para crear tu propia excepción basta con crear una clase que haga <strong>extends Exception</strong> (o de cualquier subclase).' },
    { q: '¿Qué clase se usa para representar una ruta de fichero en el sistema?', hint: 'No lee contenido, solo gestiona metadatos.',
      opts: ['FileReader', 'File', 'PathStream', 'Scanner'], ans: 1,
      exp: 'La clase <strong>File</strong> representa el nombre de un fichero o directorio y permite ver si existe, borrarlo, etc.' },
    { q: '¿Qué ventaja tiene BufferedReader sobre FileReader?', hint: 'Eficiencia.',
      opts: ['Es más fácil de usar', 'Lee línea a línea de forma eficiente usando un buffer', 'Solo lee binarios', 'No necesita cerrarse'], ans: 1,
      exp: '<strong>BufferedReader</strong> usa un buffer intermedio para reducir los accesos a disco, permitiendo leer líneas completas con readLine().' },
    { q: '¿Qué hace el parámetro "true" en "new FileWriter(\"f.txt\", true)"?', hint: 'Añadir vs machacar.',
      opts: ['Crea el fichero si no existe', 'Activa el modo append (añadir al final)', 'Bloquea el fichero para otros', 'Lo hace de solo lectura'], ans: 1,
      exp: 'El segundo parámetro booleano de <strong>FileWriter</strong> activa el modo **append**: si es true, añade al final en lugar de borrar lo que había.' },
    { q: '¿Qué interfaz debe implementar una clase para poder guardarse en un fichero con ObjectOutputStream?', hint: 'Persistencia de objetos.',
      opts: ['Cloneable', 'Runnable', 'Serializable', 'Persistent'], ans: 2,
      exp: 'Para que un objeto sea serializable, su clase debe implementar la interfaz marcador <strong>Serializable</strong>.' },
    { q: '¿Qué es el "try-with-resources"?', hint: 'Gestión automática de recursos.',
      opts: ['Un try que no necesita catch', 'Un bloque que cierra automáticamente los recursos que implementan AutoCloseable', 'Un try muy rápido', 'Un modo de depuración'], ans: 1,
      exp: '<strong>try-with-resources</strong> (Java 7+) cierra automáticamente el flujo al terminar el bloque, evitando fugas de memoria sin usar finally.' },
    { q: '¿Qué método de BufferedReader devuelve null cuando se llega al final del fichero?', hint: 'Lectura secuencial.',
      opts: ['read()', 'next()', 'readLine()', 'getLine()'], ans: 2,
      exp: '<strong>readLine()</strong> devuelve la cadena leída o **null** si no hay más datos (EOF).' },
    { q: '¿Qué palabra clave evita que un atributo se guarde durante la serialización?', hint: 'Para datos sensibles o temporales.',
      opts: ['static', 'final', 'transient', 'volatile'], ans: 2,
      exp: 'Los atributos marcados como <strong>transient</strong> son ignorados por el proceso de serialización automática.' },
    { q: '¿Para qué sirve la clase PrintWriter?', hint: 'Escritura cómoda.',
      opts: ['Para leer texto', 'Para escribir texto usando métodos como print() y println()', 'Para imprimir en papel', 'Para dibujar gráficos'], ans: 1,
      exp: '<strong>PrintWriter</strong> envuelve un flujo de escritura y permite usar los mismos métodos que System.out (print, println) para escribir en ficheros.' },
    { q: '¿Qué diferencia hay entre Thread.start() y Thread.run()?', hint: 'Clave del paralelismo.',
      opts: ['Son iguales', 'start() inicia un nuevo hilo; run() ejecuta el código en el hilo actual', 'run() inicia un nuevo hilo; start() no', 'start() es solo para Windows'], ans: 1,
      exp: '¡Vital! <strong>start()</strong> pide a la JVM crear un nuevo hilo de ejecución. Llamar a **run()** directamente es como llamar a un método normal: no hay paralelismo.' },
    { q: '¿Qué hace la palabra clave "synchronized" en un método?', hint: 'Exclusión mutua.',
      opts: ['Hace el método más rápido', 'Permite que muchos hilos entren a la vez', 'Solo permite que un hilo ejecute el método a la vez sobre ese objeto', 'Sincroniza el reloj del sistema'], ans: 2,
      exp: '<strong>synchronized</strong> garantiza que solo un hilo acceda al recurso a la vez (Monitor), evitando que los datos se corrompan por accesos simultáneos.' },
    { q: '¿Cómo se pone a dormir un hilo durante 2 segundos?', hint: 'Usa milisegundos.',
      opts: ['Thread.wait(2000)', 'Thread.sleep(2000)', 'Thread.pause(2)', 'Thread.stop(2000)'], ans: 1,
      exp: '<strong>Thread.sleep(2000)</strong> detiene el hilo actual durante 2000ms. Lanza InterruptedException.' },
    { q: '¿Qué hace el método join() de un hilo?', hint: 'Esperar a alguien.',
      opts: ['Une dos hilos en uno', 'Hace que el hilo actual espere a que el hilo sobre el que se llama termine', 'Reinicia el hilo', 'Detiene todos los hilos'], ans: 1,
      exp: 'Si haces <code>t1.join()</code>, el hilo que ejecuta esa línea se queda bloqueado hasta que **t1** finalice su ejecución.' },
    { q: '¿Cuál es la forma recomendada de crear un hilo si la clase ya hereda de otra?', hint: 'Heredar vs Implementar.',
      opts: ['Extender Thread', 'Implementar Runnable', 'No se puede crear el hilo', 'Usar static'], ans: 1,
      exp: 'Como Java no tiene herencia múltiple, si tu clase ya hereda de otra, debes **implementar Runnable** para poder convertirla en hilo.' },
    { q: '¿Qué estado tiene un hilo que ha sido creado pero aún no ha llamado a start()?', hint: 'Ciclo de vida.',
      opts: ['RUNNABLE', 'NEW', 'BLOCKED', 'WAITING'], ans: 1,
      exp: 'Un hilo recién instanciado con new está en estado <strong>NEW</strong>. Pasa a RUNNABLE tras llamar a start().' }
  ]},
