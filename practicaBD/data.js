...
const QUIZZES = {
  ...
  practica_30: { title: 'Simulacro Práctico: 30 Preguntas', questions: [
    ...
    { q: '[CASO SQL] COALESCE(columna, 0).', hint: 'Sustitución.',
      opts: ['Suma', 'Cambia NULL por 0', 'Borra nulos', 'Contar'], ans: 1,
      exp: 'Devuelve el primer valor no nulo de los argumentos.' },
    { q: '[CASO SQL] Empleado y su Jefe (ambos en la misma tabla).', hint: 'SELF JOIN.',
      opts: ['CROSS JOIN', 'SELF JOIN', 'UNION', 'No se puede'], ans: 1,
      exp: 'El SELF JOIN permite relacionar filas de la misma tabla.' }
  ]},
  sql_40_quiz: { title: 'Batería SQL: 40 Preguntas de Consultas', questions: [
    { q: '¿Qué diferencia hay entre WHERE y HAVING?', hint: 'Piensa en el orden de ejecución.',
      opts: ['No hay diferencia', 'WHERE filtra filas antes de agrupar; HAVING filtra grupos después de agrupar', 'HAVING es para DDL y WHERE para DML', 'WHERE solo se usa en SELECT'], ans: 1,
      exp: '<strong>Clave de examen</strong>: WHERE filtra registros individuales. HAVING filtra los resultados de funciones de agregado (SUM, COUNT...) tras el GROUP BY.' },
    { q: '¿Cuál es el resultado de COUNT(comision) si hay 10 filas y 3 tienen valor NULL en esa columna?', hint: 'COUNT(columna) ignora nulos.',
      opts: ['10', '3', '7', 'Error'], ans: 2,
      exp: 'COUNT(columna) cuenta solo valores no nulos. 10 - 3 = 7. Si fuera COUNT(*), devolvería 10.' },
    { q: '¿Qué JOIN usarías para ver todos los clientes, tengan o no pedidos?', hint: 'Clientes es la tabla principal.',
      opts: ['INNER JOIN', 'LEFT JOIN', 'CROSS JOIN', 'FULL JOIN'], ans: 1,
      exp: 'LEFT JOIN mantiene todas las filas de la tabla izquierda (clientes) y añade NULLs donde no hay coincidencia en la derecha (pedidos).' },
    { q: '¿Qué significa el símbolo % en una cláusula LIKE?', hint: 'Búsqueda de patrones.',
      opts: ['Representa un solo carácter cualquiera', 'Representa cualquier secuencia de cero o más caracteres', 'Representa un número', 'Indica el final de la cadena'], ans: 1,
      exp: 'El porcentaje (%) es el comodín para múltiples caracteres. El guion bajo (_) es para un solo carácter.' },
    { q: 'Para obtener la media de salarios por departamento, ¿qué cláusula es obligatoria?', hint: 'Agrupación.',
      opts: ['ORDER BY', 'SORT BY', 'GROUP BY', 'DISTINCT'], ans: 2,
      exp: 'Para usar funciones de agregado (AVG, SUM...) sobre categorías, necesitas agruparlas con GROUP BY.' },
    { q: '¿Qué operador comprueba si un valor está dentro de una lista de valores?', hint: 'Multi-valor.',
      opts: ['BETWEEN', 'IN', 'LIKE', 'EXISTS'], ans: 1,
      exp: 'IN (v1, v2, v3) es una forma abreviada y eficiente de escribir múltiples OR.' },
    { q: '¿Qué hace la sentencia DELETE FROM tabla sin WHERE?', hint: '¡Peligro!',
      opts: ['Borra la tabla de la base de datos', 'Borra todos los registros pero mantiene la estructura', 'Da un error de sintaxis', 'Solo borra el primer registro'], ans: 1,
      exp: 'Al no filtrar con WHERE, se eliminan TODOS los datos de la tabla. La estructura (columnas) permanece intacta.' },
    { q: '¿Diferencia entre TRUNCATE y DELETE?', hint: 'DDL vs DML.',
      opts: ['TRUNCATE es más lento', 'DELETE borra la estructura', 'TRUNCATE es DDL, más rápido y no permite WHERE', 'No hay diferencia'], ans: 2,
      exp: 'TRUNCATE es una operación estructural (DDL) que vacía la tabla al instante sin registrar borrados fila a fila.' },
    { q: '¿Qué devuelve SELECT * FROM A, B?', hint: 'Sin JOIN.',
      opts: ['INNER JOIN', 'Producto Cartesiano (todas las combinaciones)', 'Error', 'La unión de ambas'], ans: 1,
      exp: 'Si no hay condición de unión, cada fila de A se combina con cada fila de B. 10x5 = 50 filas.' },
    { q: '¿Cuál es el orden lógico de ejecución de una consulta SQL?', hint: 'Empieza por los datos.',
      opts: ['SELECT > FROM > WHERE', 'FROM > WHERE > GROUP BY > SELECT', 'WHERE > SELECT > FROM', 'SELECT > WHERE > GROUP BY'], ans: 1,
      exp: 'Primero se localiza la tabla (FROM), se filtran filas (WHERE), se agrupan (GROUP BY) y finalmente se eligen columnas (SELECT).' },
    { q: '¿Cómo se comprueba si un valor es nulo?', hint: 'No uses =.',
      opts: ['col = NULL', 'col IS NULL', 'col != NULL', 'col EXISTS NULL'], ans: 1,
      exp: 'En SQL, NULL no es un valor comparable con operadores normales. Debes usar IS NULL o IS NOT NULL.' },
    { q: '¿Qué hace DISTINCT?', hint: 'Unicidad.',
      opts: ['Ordena los datos', 'Elimina filas duplicadas del resultado', 'Cuenta los nulos', 'Filtra por clave'], ans: 1,
      exp: 'DISTINCT asegura que cada fila devuelta sea única en el conjunto de resultados.' },
    { q: '¿Qué operador se usa para rangos (ej: sueldo entre 1000 y 2000)?', hint: 'Inclusivo.',
      opts: ['RANGE', 'IN', 'BETWEEN', 'LIMIT'], ans: 2,
      exp: 'BETWEEN x AND y incluye tanto x como y. Es equivalente a >= x AND <= y.' },
    { q: '¿Qué es una clave foránea (Foreign Key)?', hint: 'Referencia.',
      opts: ['La clave que más se usa', 'Un atributo que referencia a la PK de otra tabla', 'Una clave que no se puede repetir', 'Un índice de búsqueda rápida'], ans: 1,
      exp: 'Sirve para mantener la integridad referencial vinculando filas de distintas tablas.' },
    { q: '¿Qué hace ON DELETE CASCADE?', hint: 'Efecto dominó.',
      opts: ['Impide borrar si hay hijos', 'Borra los hijos si el padre es borrado', 'Pone NULL en los hijos', 'Lanza un trigger'], ans: 1,
      exp: 'Mantiene la limpieza de la BD borrando registros dependientes automáticamente.' },
    { q: 'Diferencia UNION vs UNION ALL.', hint: 'Duplicados.',
      opts: ['UNION ALL elimina duplicados', 'UNION elimina duplicados; UNION ALL es más rápido porque no los comprueba', 'Solo varían en el nombre', 'UNION es para tablas distintas'], ans: 1,
      exp: 'Usa UNION ALL si sabes que no hay duplicados o no te importa verlos, para ahorrar tiempo de proceso.' },
    { q: '¿Qué es una subconsulta?', hint: 'Consulta anidada.',
      opts: ['Una consulta muy corta', 'Una consulta dentro de otra sentencia SQL', 'Una consulta a una tabla pequeña', 'Un alias de tabla'], ans: 1,
      exp: 'Permiten realizar filtros dinámicos basados en otros resultados de búsqueda.' },
    { q: '¿Qué hace la función COALESCE(col, 0)?', hint: 'Manejo de nulos.',
      opts: ['Suma 0 a la columna', 'Devuelve 0 si la columna es NULL', 'Filtra los ceros', 'Cuenta nulos'], ans: 1,
      exp: 'Devuelve el primer valor no nulo de la lista. Muy útil para informes donde no quieres ver campos vacíos.' },
    { q: '¿Para qué sirve el operador EXISTS?', hint: 'Presencia.',
      opts: ['Comprueba si una tabla existe', 'Verifica si una subconsulta devuelve al menos una fila', 'Crea una tabla', 'Busca nulos'], ans: 1,
      exp: 'Es más eficiente que IN en subconsultas grandes porque se detiene en cuanto encuentra la primera coincidencia.' },
    { q: '¿Qué hace ROLLBACK?', hint: 'Transacción.',
      opts: ['Cierra la base de datos', 'Deshace todos los cambios no confirmados de la transacción', 'Confirma los cambios', 'Crea una copia de seguridad'], ans: 1,
      exp: 'Permite recuperar el estado anterior si algo falla durante una serie de operaciones (ej: transferencia bancaria).' },
    { q: '¿Qué hace COMMIT?', hint: 'Persistencia.',
      opts: ['Borra la transacción', 'Confirma y guarda permanentemente los cambios', 'Crea un punto de restauración', 'Finaliza la conexión'], ans: 1,
      exp: 'Una vez hecho COMMIT, los cambios son visibles para todos los usuarios y persistentes.' },
    { q: '¿Qué es un SELF JOIN?', hint: 'Uno mismo.',
      opts: ['Unir dos tablas iguales', 'Unir una tabla consigo misma usando alias distintos', 'Unir todas las tablas del esquema', 'Unir por clave primaria'], ans: 1,
      exp: 'Se usa para jerarquías (ej: tabla EMPLEADOS donde una columna es id_jefe que apunta a otro empleado).' },
    { q: '¿Qué cláusula limita el número de filas devueltas (en MySQL/PostgreSQL)?', hint: 'Paginación.',
      opts: ['TOP', 'STOP', 'LIMIT', 'COUNT'], ans: 2,
      exp: 'LIMIT 10 devuelve solo las 10 primeras filas. Muy usado para paginar resultados en webs.' },
    { q: '¿Qué hace UPDATE empleados SET salario = 2000;?', hint: '¡Sin WHERE!',
      opts: ['Sube el salario a todos a 2000', 'Error por falta de filtro', 'Cambia solo al primer empleado', 'Actualiza solo si el salario era menor'], ans: 0,
      exp: '¡Cuidado! Sin WHERE, SQL aplica el cambio a CUALQUIER registro de la tabla.' },
    { q: '¿Cuál es la diferencia entre CHAR(10) y VARCHAR(10)?', hint: 'Longitud fija vs variable.',
      opts: ['CHAR siempre ocupa 10 bytes; VARCHAR solo lo que ocupa el texto', 'VARCHAR es solo para números', 'CHAR es más lento', 'No hay diferencia real'], ans: 0,
      exp: 'CHAR rellena con espacios hasta completar el tamaño; VARCHAR es más flexible con el espacio en disco.' },
    { q: '¿Para qué sirve AS?', hint: 'Alias.',
      opts: ['Para comparar valores', 'Para asignar un nombre temporal (alias) a una columna o tabla', 'Para ordenar resultados', 'Para filtrar nulos'], ans: 1,
      exp: 'Mejora la legibilidad de las cabeceras de los informes o acorta nombres de tablas en JOINs.' },
    { q: '¿Qué función de agregado usarías para saber el total facturado?', hint: 'Suma.',
      opts: ['COUNT()', 'TOTAL()', 'SUM()', 'AVG()'], ans: 2,
      exp: 'SUM(columna) suma todos los valores numéricos de la columna (ignorando nulos).' },
    { q: '¿Qué hace SELECT UPPER(nombre)?', hint: 'Transformación.',
      opts: ['Muestra el nombre en negrita', 'Convierte el texto a mayúsculas', 'Busca nombres de directivos', 'Ordena alfabéticamente'], ans: 1,
      exp: 'Función escalar que transforma el valor de la fila antes de mostrarlo.' },
    { q: '¿Qué operador combina resultados de dos consultas quedándose solo con los COMUNES?', hint: 'Intersección.',
      opts: ['UNION', 'INTERSECT', 'EXCEPT', 'MINUS'], ans: 1,
      exp: 'Devuelve las filas que aparecen en ambos conjuntos de resultados.' },
    { q: '¿Qué operador resta los resultados de una consulta a otra?', hint: 'Diferencia.',
      opts: ['UNION', 'INTERSECT', 'EXCEPT (o MINUS)', 'DELETE'], ans: 2,
      exp: 'Devuelve filas de la primera consulta que NO están en la segunda.' },
    { q: '¿Qué comando DDL añade una columna a una tabla?', hint: 'ALTER.',
      opts: ['UPDATE TABLE ADD', 'ALTER TABLE ADD', 'CHANGE TABLE', 'INSERT COLUMN'], ans: 1,
      exp: 'ALTER TABLE se usa para modificar la estructura del "contenedor" (la tabla).' },
    { q: '¿Qué tipo de dato es mejor para una fecha de nacimiento?', hint: 'Temporal.',
      opts: ['VARCHAR', 'INT', 'DATE', 'TIMESTAMP'], ans: 2,
      exp: 'DATE guarda Año-Mes-Día. TIMESTAMP o DATETIME incluyen además la hora.' },
    { q: '¿Qué restricción impide que una columna tenga valores nulos?', hint: 'Obligatorio.',
      opts: ['REQUIRED', 'PRIMARY KEY', 'NOT NULL', 'UNIQUE'], ans: 2,
      exp: 'NOT NULL obliga a que el campo tenga siempre un valor al insertar o actualizar.' },
    { q: '¿Qué comando elimina una tabla físicamente?', hint: 'DROP.',
      opts: ['DELETE TABLE', 'REMOVE TABLE', 'DROP TABLE', 'TRUNCATE TABLE'], ans: 2,
      exp: 'DROP borra la definición de la tabla y todos sus datos. Es irreversible sin backup.' },
    { q: '¿Qué es un Producto Cartesiano?', hint: 'CROSS JOIN.',
      opts: ['El resultado de unir PK con FK', 'Todas las combinaciones posibles entre dos tablas', 'Una tabla de productos', 'Un error de JOIN'], ans: 1,
      exp: 'Ocurre cuando no hay condición de unión. Suele ser un error porque genera muchísimas filas.' },
    { q: '¿Cómo se concatenan cadenas en SQL (estándar)?', hint: 'Unir texto.',
      opts: ['col1 + col2', 'CONCAT(col1, col2)', 'col1 & col2', 'col1 . col2'], ans: 1,
      exp: 'CONCAT o el operador || (según el motor) permiten unir textos de distintas columnas.' },
    { q: '¿Qué hace la cláusula ORDER BY 1 DESC?', hint: 'Referencia por posición.',
      opts: ['Ordena por la primera columna de forma descendente', 'Ordena el primer registro', 'Error', 'Ordena por ID'], ans: 0,
      exp: 'Se puede referenciar a las columnas por su posición en el SELECT, aunque es mejor usar el nombre.' },
    { q: '¿Qué hace SELECT * FROM empleados WHERE comision IS NOT NULL?', hint: 'Filtro de presencia.',
      opts: ['Muestra empleados sin comisión', 'Muestra solo empleados que tienen asignada una comisión', 'Error', 'Muestra el total de comisiones'], ans: 1,
      exp: 'IS NOT NULL filtra para excluir los registros que no tienen dato en esa columna.' },
